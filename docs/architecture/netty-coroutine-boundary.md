...
| Work type | Thread | Reason |
|---|---|---|
| VarInt frame decoding | Netty event-loop | Inline in pipeline, zero allocations |
| AES cipher (future) | Netty event-loop | Must complete before frame decode |
| zlib compress/decompress | Netty event-loop | Inline in pipeline, before codec |
| Packet decode/encode | Netty event-loop | Must use the channel's `ByteBuf` allocator |
| State machine transitions | Netty event-loop | Atomic with the triggering packet |
| Mojang session auth | Coroutine (IO/Default) | HTTP — blocks; must not stall other connections |
| Plugin event handlers | Coroutine (Default) | Arbitrary plugin code — may be slow |
| Database reads/writes | Coroutine (IO) | Blocking I/O |
| Scheduled tasks (`delay`) | Coroutine (Default) | Timer-based, not I/O |
| Plugin load/enable | Coroutine (Default) | Parallel classloading; CPU-bound |
...
| ✅ Safe | ❌ Unsafe |
|---|---|
| `proxyScope.launch { }` from a session handler | `Thread.sleep()` on a Netty thread |
| `channel.writeAndFlush()` from any thread | Calling `pipeline().addLast()` from a coroutine |
| `channel.eventLoop().execute { }` from a coroutine | Reading a `ByteBuf` after its reference count drops to zero |
| `delay()` inside a coroutine | Blocking network call inside a session handler callback |
| `withContext(Dispatchers.IO) { }` for blocking work | Sharing a non-thread-safe `ByteBuf` across threads |
